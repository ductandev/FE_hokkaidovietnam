...
function Footer() {
    ...
    const headText = (text: string, customClass: string) => {
        return <h2 className={`uppercase lg:text-2xl text-base font-medium ${customClass}`}>{text}</h2>
    }

    const linkText = (type: string, customClass?: string) => {
        return <div className="flex flex-col">
            {linkMapping[type].map((item: any, id: any) => {
                return <Link className={`mb-3 text-secondary lg:text-base text-sm  ${customClass}`} to={item.link} key={id}>{item.label}</Link>
            })}
        </div>
    };
